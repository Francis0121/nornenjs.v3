...
JpegEncoder::encode_tj()
{
    ...
    switch (buf_type) {
        case BUF_RGBA:
            compTest(chandle, &jpeg, &jpeg_len, width, height, TJPF_RGBA, TJSAMP_444, quality, 2048, data);
            break;

        case BUF_BGRA:
           compTest(chandle, &jpeg, &jpeg_len, width, height, TJPF_BGRA, TJSAMP_444, quality, 2048, data);
            break;
