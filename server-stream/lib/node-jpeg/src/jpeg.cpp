...
Jpeg::Initialize(v8::Handle<v8::Object> target)
{
    ...
    NODE_SET_PROTOTYPE_METHOD(t, "turboencodeSync", TurboJpegEncodeSync);
...
Handle<Value>
Jpeg::TurboJpegEncodeSync(const Arguments &args)
{
    HandleScope scope;
    Jpeg *jpeg = ObjectWrap::Unwrap<Jpeg>(args.This());
    return scope.Close(jpeg->TurboJpegEncodeSync());
}
Handle<Value>
